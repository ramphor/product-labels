...
foreach ( $posts_array as $post_id ) {
	...
	echo '<p><label><input name="br_labels[label_from_post][]" type="checkbox" value="' . $post_id->ID . '"' . ( is_array( $label['label_from_post'] ) && in_array( $post_id->ID, $label['label_from_post'] ) ? ' checked' : '' ) . '>(' . $post_id->ID . ') ' . $post_title . '</label></p>';
}
