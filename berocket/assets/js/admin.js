	...
	$( document ).ready(
		function () {
			...
			function destroy_br_saved() {
				...
				$( '.br_framework_settings ul.side a' ).click(
					function(event) {
				...
				br_init_colorpick = function() {
					$( '.br_framework_settings .br_colorpicker' ).each(
						function (i,o){
				...
				$( document ).on(
					'click',
					'.br_framework_settings .br_colorpicker_default',
					function (event) {
						event.preventDefault();
				...
				$( document ).on(
					'click',
					'.br_framework_settings .berocket_upload_image',
					function(e) {
						e.preventDefault();
						$p                  = $( this );
				...
				$( document ).on(
					'click',
					'.br_framework_settings .berocket_remove_image',
					function(event) {
						event.preventDefault();
				...
				$( document ).on(
					'click',
					'.berocket_select_fontawesome .berocket_select_fa',
					function(event) {
						event.preventDefault();
				...
				$( document ).on(
					'hover',
					'.berocket_select_fontawesome .berocket_fa_hover',
					function() {
				...
				$( document ).on(
					'click',
					'.berocket_select_fontawesome .berocket_remove_fa',
					function(event) {
						event.preventDefault();
				...
				$( document ).on(
					'click',
					'.berocket_select_fontawesome .berocket_fa_dark .berocket_fa_close',
					function(event) {
						event.preventDefault();
			...
			function berocket_save_button_side() {
				...
				if ( $button.length ) {
					...
					berocket_save_button_side_timeout = setTimeout(
						function() {
							/*Settings to set*/
							var bottom   = '0px';
							var position = 'fixed';
...
(function ($){
	$( document ).ready(
		function () {
				...
				$( document ).on(
					'click',
					'.berocket_products_search .button',
					function(event) {
						event.stopPropagation();
						...
						delay_search    = setTimeout(
							function () {
								if ( $search_block.val().length >= 3 && $search_block.val() != last_search ) {
									  ...
									  var exists  = [];
									$search_box.find( '.berocket_product_selected input' ).each(
										function( i, o ) {
											exists.push( $( o ).val() );
										}
									);
...
function berocket_blocks_messages(options) {
	...
	if ( ! berocket_block_messages_elements.length && berocket_block_messages_element_last == false ) {
		...
		berocket_block_messages_element_interval = setInterval(
			function() {
...
(function ($){
	$( document ).ready(
		function () {
				$( document ).on(
					'click',
					'.berocket_post_set_new_sortable, .berocket_post_set_new_sortable_set',
					function(event) {
						event.preventDefault();
						var $this   = $( this );
						...
						$.post(
							location.href,
							{braction:'berocket_custom_post_sortable', BRsortable_id:post_id, BRorder:order},
							function(html) {
...
jQuery( document ).on(
	'click',
	'.berocket_templates_list .berocket_template_label .berocket_template_paid_get a',
	function(event) {
...
jQuery( document ).on(
	'click',
	'.berocket_addons_list .berocket_addon_label .berocket_addon_paid_get a',
	function(event) {
		event.stopPropogation();
	}
);
